...
def part_5():
    ...
    # Helper code
    def coords_particle_filter(filter_class, imgs_dir, template_rect,
                            save_frames={}, **kwargs):
        out_ls = []
        imgs_list = [f for f in os.listdir(imgs_dir)
                     if f[0] != '.' and f.endswith('.jpg')]
        ...
        template = None
        pf = None
        frame_num = template_rect['from_frame']#template_rect['from_frame']
        # Loop over video (till last frame or Ctrl+C is presssed)
        for img in imgs_list[template_rect['from_frame']:]:
            frame = cv2.imread(os.path.join(imgs_dir, img))
            # Extract template and initialize (one-time only)
            if template is None:
                template = frame[int(template_rect['y']):
            ...
            # Render and save output, if indicated
            if frame_num in save_frames.keys():
                print frame_num
                frame_out = frame.copy()
                pf.render(frame_out)
                #cv2.imwrite(save_frames[frame_num], frame_out)
                out_ls.append(frame_out)
            # Update frame number
            frame_num += 1
        return out_ls
    
    def merge_image(imgs):
        cols = [ (0, 0, 255), (255, 255, 255)]
        replace = [0, 128, 255]
        out_img = imgs[0].copy()
        for i, img in enumerate(imgs):
            if img=='skip':
                continue
            out_img[img==cols[0]] = img[img==cols[0]]
            out_img[img==cols[1]] = replace[i]
        return out_img
            
    tr1 = {'x': 280, 'y': 160, 'w': 100, 'h': 150, 'algorithm':ps5.ParticleFilter, 'from_frame':28,
                      'num_particles': 200, 'sigma_mse':5, 'sigma_dyn':10, 'alpha':0.2}
    tr2 = {'x': 120, 'y': 180, 'w': 50, 'h': 200, 'algorithm':ps5.AppearanceModelPF,'from_frame':28,
                      'num_particles': 200, 'sigma_mse':10, 'sigma_dyn':10, 'alpha':0.5}
    tr3 = {'x': 0, 'y': 160, 'w': 80, 'h': 170, 'algorithm':ps5.ParticleFilter,'from_frame':28,
                      'num_particles': 200, 'sigma_mse':5, 'sigma_dyn':10, 'alpha':0.2}

    '''
    base_img = cv2.imread(os.path.join(os.path.join(input_dir, "TUD-Campus"), '000001.jpg'))
    def box(tr):
        return tr['y'], tr['y']+tr['h'], tr['x'], tr['x']+tr['w']
    h1, h2, w1, w2 = box(template_rect1)
        
    cv2.imshow('Tracking', base_img[h1: h2, w1:w2] )
    cv2.waitKey(0)
    '''
    save_frames = {29-1: os.path.join(output_dir, 'ps5-5-a-1.png'),
                   56-1: os.path.join(output_dir, 'ps5-5-a-2.png'),
                   71-1: os.path.join(output_dir, 'ps5-5-a-3.png')}
    all_objs = [coords_particle_filter(tr['algorithm'],  # particle filter model class
                        os.path.join(input_dir, "TUD-Campus"),
                        tr,
                        save_frames,
                        num_particles=tr['num_particles'], sigma_exp=tr['sigma_mse'],
                        sigma_dyn=tr['sigma_dyn'], alpha=tr['alpha'],
                        template_coords=tr) for tr in [tr1, tr2, tr3]]#[template_rect1, template_rect2, template_rect3]]
    #all_objs[2] += ['skip'] 
    for t, imgs in enumerate( zip(*all_objs)):
        print t, len(imgs)
        #print t, os.path.join(output_dir, 'ps5-5-a-{}.png'.format(t+1))
        cv2.imwrite(os.path.join(output_dir, 'ps5-5-a-{}.png'.format(t+1)), merge_image(imgs))
